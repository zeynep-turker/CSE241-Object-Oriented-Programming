    ...
    if(rOuter != nullptr){
        if(rInner != nullptr){          
            ...
            for(i=0; i<lineNum; ++i)
            {
                ...
                {
                    Polygon polyShape(*rInner);
                    polygon->push_back(polyShape);
        ...
        if(rInner != nullptr){
           ...
           for(i=0; i<lineNum/2; ++i){
            ...
            for(i=0; i<(coloumnNum-1)/2; ++i){
                ...
                cInner->setPositionY(cOuter->getSize());
                Polygon polyShape2(*cInner);
                polygon->push_back(polyShape2);
                b=cOuter->getSize()+(i+1)*2*cInner->getSize();
                cInner->setPositionX(b);
                cInner->setPositionY(cOuter->getSize());
                Polygon polyShape3(*cInner);
                polygon->push_back(polyShape3);
            }
            ...
            for(i=0; i<lineNum; ++i){
                ...
                for(j=0; j<(int)(coloumnNum-1)/2; ++j){
                    ...
                    cInner->setPositionY(a);
                    Polygon polyShape4(*cInner);
                    polygon->push_back(polyShape4);
                    cInner->setPositionX(c);
                    cInner->setPositionY(h);
                    Polygon polyShape5(polyShape5);
                    polygon->push_back(*cInner);
                    cInner->setPositionX(d);
                    cInner->setPositionY(h);
                    Polygon polyShape6(*cInner);
                    polygon->push_back(polyShape6);
                }
            }           
        }
        else{
            tInner->setPositionT1(cOuter->getSize(),cOuter->getSize()-(((tInner->getSize()/2)*sqrt(3))/2));
            tInner->setPositionT2(cOuter->getSize()-(tInner->getSize()/2),cOuter->getSize()+((tInner->getSize()/2)*sqrt(3))/2);
            tInner->setPositionT3(cOuter->getSize()+(tInner->getSize()/2),cOuter->getSize()+((tInner->getSize()/2)*sqrt(3))/2);
            Polygon polyShape=*tInner;
            polygon->push_back(polyShape);
...
ostream& operator<<(ostream &out, const ComposedShape &shape){
    ...
    out<<shape.getPolygonVector();//Polygon vectoru svg olarak dosyaya yazılır.
